...
export class Game extends RoomBase {
  ...
  start() {
    ...
    this._tick(() => {
      this.deck = this.application.cards.generateDecks(this.options.sets);
  ...
  addCardToStack(card, stackOrStackId) {
    ...
    if (this.step != A.STEP_CHOOSE_WHITES)
      return Validator.fail('You cannot do that now');

    if (this.round.isStackFinished(stackOrStackId))
      return Validator.fail('The stacks already finished');

    this._tick(() => {
      this.round.addCardToStack(card, stackOrStackId);
    });
  ...
  _nextRound(wasCanceled = false) {
    ...
    if (this.round) {
      if (wasCanceled) {
        for (const stack of this.round.stackList)
    ...
    const czar = this._nextCzarIndex ? this.players[this._nextCzarIndex % this.players.length] : this.players[0];

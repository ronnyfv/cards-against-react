...
export function makeDiffArray(before, after, location = '') {
  ...
  if (after.length == 0) {
    return (before && before.length) ? { $splice: [0, before.length] } : IS_UNCHANGED;
  }
  ...
  if (!after[0].hasOwnProperty('id')) {
    ...
    const itemsBefore = before == null ? {} : before.reduce((obj, item) => {
      obj[item.id] = item;
      return obj;
    }, {});
    ...
    return sameCount == after.length && before.length == after.length ?
      { $update: mutatedItems } : { $update: mutatedItems, ids: idOrder };
  }
}
...
export function mergeDiff(base, diff, location = '') {
  ...
  if (diff.hasOwnProperty('$update')) {
    ...
    if (idOrder) {
      ...
      const oldItems = !base ? {} : base.reduce((obj, item) => {
        obj[item.id] = item;
        return obj;
      }, {});
      ...
      for (let i = 0; i < base.length; i++) {
        const item = base[i];
        result[i] = updateItems.hasOwnProperty(item.id) ? mergeDiff(base[i], updateItems[item.id]) : base[i];
      }

...
class LoginDialog extends ContainerBase {
  ...
  componentWillMount() {
    ...
    this.subscribe(user.opLogin$, (opLogin) => {
      this.setState({ opLogin });
    });
  ...
  render() {
    ...
    return (
      <section className="comp-login-dialog">
        <h1 className="m-b-20">Login</h1>
        <form onSubmit={this._login} disabled={disabled}>
          <div className="form-row">
            <TextInput placeholder="username" ref={(c) => this._username = c} disabled={disabled || !opLogin.can} />
          </div>
          {!opLogin.error ? null : <p className="error">{opLogin.error}</p>}
          <div className="submit-row m-t-20">
            <button className="m-button good" disabled={disabled || !opLogin.can}>login</button>
            <button className="m-button close-button" onClick={this._close}>cancel</button>
          </div>
        </form>
      </section>
    );
